...
pub fn game(pname: String, num_rounds: usize) {
    ...
    while !done {
        ...
        ghistory.insert(
            nround,
            GameSum::new(g.clone(), vec![robo.clone(), player.clone()]),
        );
...
fn get_daemons<'a>(p: &'a mut Player, r: &'a mut Player) -> (&'a mut Player, &'a mut Player) {
...
fn markovp(game_history: &HashMap<usize, GameSum>, nthround: usize) -> HashMap<String, f32> {
    ...
    let ord: usize = 2;
    let pmat = markov::tranprob(&dat, ord);
    pmat
}
